...
> **免責**: 本プロジェクトは、株式会社タカラトミー様および関連各社とは一切関係のない、非公式・非営利のファン制作物です。コードは MIT License ですが、同梱のカードデータ（カード名・効果テキスト等）の権利は各権利者に帰属し、MIT License の対象外です。詳細は [ライセンスと免責](#ライセンスと免責) / [NOTICE](NOTICE) を参照してください。
...
本リポジトリには、性質の異なる2種類のものが含まれます。

- **ソースコード（ゲームエンジン本体）**: [MIT License](LICENSE) で提供します。
- **カードデータ**（`data/impl_cards/`・`data/impl_card_metadata/` のカード名・効果テキスト・レアリティ・セット記号など）: トレーディングカードゲーム「デュエル・マスターズ」に由来し、著作権・商標その他の権利は **株式会社タカラトミーおよび各権利者に帰属** します。これらは MIT License の対象には **含まれません**。

本プロジェクトは、株式会社タカラトミー様および関連各社とは一切関係のない、**非公式・非営利のファン制作物**です。カードデータはエンジンの動作検証・研究を目的として収録しています。「デュエル・マスターズ」は株式会社タカラトミーの商標です。権利者からの申し出があった場合、該当データは速やかに削除します。
