...
`abilities.keyword` グループに、registry に登録されていないキーワード ID がある場合に出ます。キーワードは文字列（`"blocker"`）でもオブジェクト（`{"ability_id": "power_attacker", ...}`）でも書けますが、その ID が未登録だと **`validate` / `make` は通る一方、実際にそのカードの能力を生成する時（プレイ・召喚時）に `create_ability` が `ValueError: Unknown ability id` を投げてゲームが落ちます**。タイプミス（`blockerr`）や未実装キーワードの先行記述を、ランタイムまで遅延させずレビュー時に拾います。

既知キーワード集合は `abilities/registry.py` の `ABILITY_BUILDERS`（実体を持つ keyword）と `ABILITY_METADATA`（reminder 等メタのみの keyword）の和です（`card_definition_validator.py` の `_known_keyword_ids()` が遅延 import してキャッシュ）。新しいキーワードを実装したら `ABILITY_BUILDERS` に登録すれば自動的に既知扱いになります。

card_type が error なのに対しこちらを warning にしているのは、未登録キーワードはランタイムで明示的に例外化して気付ける（card_type 誤記の「候補が空＝沈黙」とは異なり静かに失敗しない）ため、`validate` の終了コードは成功のまま早期検出だけ行えば十分だからです。
